...
class ParaTest < Minitest::Test
  ...
  def test_file_encoder
    ...
    assert_equal e.encode(example_string).length, fe.encode("example.txt", "encoded_example.txt")
  end
  ...
  def test_file_decoder
    ...
    assert_equal d.decode(example_string).length, fd.decode("encoded_example_copy.txt", "decoded_example.txt")
  end
